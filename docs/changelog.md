...
## [1.3.0] - 2026-09-09
- 既定モデル更新: `gpt-5.6-sol` から `gpt-6-astra` に変更。推論強度・詳しさの既定値はともに `medium`
- モデル対応: GPT-6 系にも `reasoning.effort` と `text.verbosity` の設定を送信するよう対応
- 設定サンプル更新: 全3プロファイルを `gpt-6-astra` に統一し、標準回答は `medium`、詳細分析は `high`、高速回答は `low` の推論強度・詳しさに設定
- 依存更新: `openai` を `7.10.0` から `7.12.1` に更新
- 依存更新: `@types/node` を `26.4.1` から `26.5.0` に更新し、間接依存の `undici-types` を `8.3.0` から `8.9.0` に更新
- テスト/CI: モデル対応の回帰テストを `test:models` に統一し、Astra の既定値、全3ツール、YAML / ENV のモデル指定、対応パラメーターの送信、実モデルIDを検証
- ドキュメント: 既定モデルの案内と設定例を Astra に同期し、既存スクリプトで実際に質問・回答を確認する手順を明記
- ドキュメント: Node.js の配布要件 `>=24` と、CI / Release の実行環境24系を区別して説明

## [1.2.2] - 2026-09-07
- 依存更新: `openai` を `7.4.0` から `7.10.0` に更新
- 依存更新: `@types/node` を `26.2.0` から `26.4.1` に更新
- 依存更新: `tsx` を `4.23.12` から `4.23.13` に更新し、間接依存の `esbuild` を `0.28.0` から `0.28.2` に更新
- 依存管理: `deps:check` / `deps:update` で直接・間接依存と脆弱性・スクリプト承認を確認し、直接依存に更新がない場合の間接依存の更新漏れを修正
- ビルド: `esbuild` / `fsevents` のスクリプト承認をパッケージ名単位に変更し、更新時の承認警告を解消。新規の未承認スクリプトは実行前に停止
- 開発環境: ソースからの導入・ビルドに必要な npm を 11.19.0 以上に変更
- CI/CD: 開発依存を含む脆弱性監査を追加し、High / Critical が残る場合は検証・公開を停止
- ドキュメント: 採番と変更履歴の基準を正準仕様に集約し、機能変更と保守修正の判断基準を整理

## [1.2.1] - 2026-08-15
- 依存関係: npm レジストリに依存する参照先 URL を `package-lock.json` に記録しない設定を追加
- リリース: npm 12 が別ホストの tarball URL を拒否して `npm ci` が失敗する問題を解消

## [1.2.0] - 2026-08-15
- 既定モデル更新: 単一プロファイルの既定値を `gpt-5.6-sol` / `medium` に更新
- 設定機能: `reasoning_effort` の許可値に `none` と `max` を追加し、定義と検証を一元化
- 応答契約: 要求したエイリアスではなく Responses API の `response.model` を優先して返し、実際に処理したモデルを記録
- 設定サンプル更新: 詳細分析=`gpt-5.6-sol`、標準回答=`gpt-5.6-terra`、高速回答=`gpt-5.6-luna` に同期。`gpt-5.6-pro` はモデルIDではないため使用しない
- テスト/CI: GPT-5.6 の既定値、全推論強度、3モデル、実モデルIDを API 通信なしで検証する回帰テストを追加
- ドキュメント: README、正準仕様、設定・導入・環境・クライアント・トランスポート・再現性・検証資料を GPT-5.6 仕様へ更新
- 依存関係: 現行 OpenAI SDK が必要な Responses API の型と値をサポート済みのため変更なし
...
- 依存更新: `openai` を `6.27.0` から `6.34.0` に更新
- 依存更新: `yaml` を `2.8.2` から `2.8.3` に更新
- 依存更新: `@types/node` を `25.4.0` から `25.6.0` に更新
- 依存更新: `typescript` を `5.9.3` から `6.0.3` に更新
